...
{
public:
	static IndexedTriangleList MakeTesselated(DXexp::VertexLayout layout, int latDiv, int longDiv)
	{
		...
		DXexp::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<DXexp::VertexLayout> layout = std::nullopt)
	{
		using Element = DXexp::VertexLayout::ElementType;
		if (!layout)
		{
			layout = DXexp::VertexLayout{}.Append(Element::Position3D);
		}
